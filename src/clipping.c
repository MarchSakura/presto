...
// good_chan_levels (which must be pre-allocated).
{
    ...
    for (ii = 0; ii < ptsperblk; ii++) {
        ...
        for (jj = 0; jj < numchan/AVXNFLOAT; jj++) {
            _sum = _mm512_add_ps(_sum, _mm512_load_ps(powptr));
            powptr += AVXNFLOAT;
        }
    ...
    /* really strong RFI from the calculation.            */
    {
        ...
        for (jj = 0; jj < numchan; jj+=AVXNDOUBLE) {
            _mm512_store_pd(chan_avg_temp+jj, _zero);
        }
        #else
        for (jj = 0; jj < numchan; jj++)
        ...
        for (ii = 0; ii < ptsperblk; ii++) {
            if (zero_dm_block[ii] > lo_cutoff && zero_dm_block[ii] < hi_cutoff) {
                ...
                #if USEAVX
                for (jj = 0; jj < numchan; jj+=AVXNDOUBLE) {
                    _mm512_store_pd(chan_avg_temp+jj, 
                                    _mm512_add_pd(_mm512_load_pd(chan_avg_temp+jj), 
                                                  _mm512_cvtps_pd(_mm256_load_ps(powptr))));
                    powptr += AVXNDOUBLE; 
                }
                #else
                for (jj = 0; jj < numchan; jj++) {
                    chan_avg_temp[jj] += *powptr++;
                    printf("see the double: %lf\n", chan_avg_temp[jj]);
                }
                #endif
                numgoodpts++;
            }
        ...
        if (numgoodpts < 1) {
            ...
            for (jj = 0; jj < numchan; jj+=AVXNDOUBLE) {
                _mm512_store_pd(chan_avg_temp+jj, 
                                _mm512_castps_pd(_mm512_load_ps(chan_running_avg+jj))); 
            }
            #else
            for (jj = 0; jj < numchan; jj++)
            ...
            current_std = sqrt(current_std);
            #if USEAVX
            __m512d _numgoodpts = _mm512_set1_pd((double)numgoodpts); 
            for (jj = 0; jj < numchan; jj+=AVXNDOUBLE) {
                _mm512_store_pd(chan_avg_temp+jj,
                                _mm512_div_pd(_mm512_load_pd(chan_avg_temp+jj), _numgoodpts));
            }
            #else 
            for (jj = 0; jj < numchan; jj++)
